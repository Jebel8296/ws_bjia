 ...
 */
'use strict'
zhongmai.controller('AddressCtrl',function($scope,$rootScope,$http,localStorageService,$timeout,$window,$state){
    ...
    $scope.openAddrLayer = function(item){
        if (item == 'new') {
            $scope.title = "新增地址";
            $scope.addressitem={name:"",province:"",city:"", area:'',address:'',phone:'',zipcode: ''}
        }else {
            $scope.title = "修改地址";
           // console.log($scope.addrlist);
    ...
    $scope.addOrUpdAddress = function (item) {
        var self = this;
       //var  text = (item.province + item.city + item.district) || '��ѡ���ַ...'
        if(item.status == true){ item.status = 1; }else{ item.status = 0; }

        if(self.addressForm.$valid){
            if(item.eid){
                //修改
                var proquery = {"eid":item.eid, "name":item.name,"phone":item.phone,"zipcode":item.zipcode,"address":item.address,"province":item.province, "city":item.city, "area":item.district,"def":item.status}
                var json ={
                ...
                //增加
                var proquery = {"uid":$rootScope.uid, "name":item.name,"phone":item.phone,"zipcode":item.zipcode,"address":item.address,"province":item.province, "city":item.city, "area":item.district,"def":item.status}
                var json ={
    ...
    $scope.deladdr = function(eid){
        var json ={
            ...
            "reqData":{"uid": $rootScope.uid,"eid":eid}
        };
    ...
    $scope.setDef = function(item){
        var json ={
            ...
            "reqData":{"uid": $rootScope.uid,"eid":item.eid}
        };
        //post
        $http({
            method : 'POST',
...
/*
    ...
    $scope.selectedWhich = function (addr) {
        $scope.selectedRow = addr;
    }
 */
