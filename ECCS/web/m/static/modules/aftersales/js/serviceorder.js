define(function(require, exports, module) {
    ...
    var orderid = parseInt(theRequest.orderid); //订单id
 	var prodcode = theRequest.prodcode; //订单编号
    var ordercode;
          ...
          var m =(new Function("","return "+msg))();
            if(m.code == 200){
                ...
                for(i=0;i<proListNum.length;i++){
                    var proList = '<li><div class="item-content">\
                          <div class="item-media"><img src="/modules/public/img/3/3C40A100010001.png"></div>\
                          <div class="item-inner">\
                            <div class="item-title-row">\
                              <div class="item-title">'+ proListNum[i].name +'</div>\
                            </div>\
                            <div class="color0">￥'+ proListNum[i].price +'</div>\
                ...
                var receiptHtml = '<dl><dt>收货地址</dt>\
                    <dd>收货人：'+ m.respData.express.name + '&nbsp;&nbsp;联系电话：'+ m.respData.express.phone +'<br/>地   址：'+ m.respData.express.provname + m.respData.express.areaname + m.respData.express.address +'</dd>\
                </dl> ';
    ...
    $(document).on('click','#subOrderBtn', function () {       
        ...
        var selAfterPro = $(".selAfter").find("label").text();

        if(typeNum.length==0){
            $.toast("请选择售后类型");
            return;
        }else if(selAfterPro.length==0){
            $.toast("请选择售后产品");
            return;
        }else {
