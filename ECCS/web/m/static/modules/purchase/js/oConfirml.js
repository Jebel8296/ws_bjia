define(function(require, exports, module) {
	...
	var id =url.substr(4);
	var code=new UrlSearch(decodeURI(url)); //实例化
	var prodcode =code.code;
	$.ajax({
        type:"post",
        url:postUrl,
        ...
        success:function(msg){
            ...
            if(m.code == 200){
				...
				if(selectINfo){
					...
					if(productItem!=""){
						...
						for(var n=0; n<productItem.length;n++){
							if(productItem[n].status ==1){
								var dfAddr = '<div id="defaultAddr" class="item-title" data-eid="'+ productItem[n].eid +'">收货人： <span class="recInfo_name">'+ productItem[n].name +'</span><br/>联系电话：<span class="recInfo_tel">'+ productItem[n].phone +'</span> <br/>\
	                  地   址：<span class="recInfo_addr" data-code="'+ productItem[n].zipcode +'>'+ productItem[n].eid +'">'+productItem[n].provname + productItem[n].cityname +productItem[n].address +'</span></div>';
							}
						}
						$("#yAddr .item-title-row").append(dfAddr);
              ...
              //$.toast(m.msg);
				$("#yAddr").show();
				var but = "<div class='text-center'>添加地址</div>"
				$("#yAddr .item-title-row").append(but);
				//$("#noAddr").show();
            }
        },
        error:function(){
        ...
        success:function(msg){
          var m =(new Function("","return "+msg))();
            ...
            if(m.code == 200){
                ...
                var productItem =m.respData;
		       	for( k=0; k<productItem.length; k++){
					productItem[k].prodprice=toDecimal2(productItem[k].prodprice)
					console.log(productItem[k].prodprice);
					var proHtml = '<li><div class="item-content" data-prodcode = "'+ productItem[k].prodcode +'"><div class="item-media"><img src="/modules/public/img/3/3C40A100010001.png"></div><div class="item-inner"><div class="item-title-row"><div class="item-title">'+ productItem[k].prodname +'</div><div class="item-after">x1</div></div><div class="item-subtitle">型号：'+ productItem[k].model +' 颜色：'+ productItem[k].colorname +'</div><div class="item-text"><span class="color0">'+ productItem[k].prodprice +'</span></div></div></div></li>';
					$(".commodityList ul").append(proHtml);
					total += +(1 * productItem[k].prodprice)
					total=toDecimal2(total)
					$('.sumPrice').text(productItem[k].prodprice);
				}
				var sumTotal=toDecimal2(parseFloat($(".freight").text())+parseFloat($('.sumPrice').text()));
				$('.allTotal').text(sumTotal);
				//商品清单 总计信息
	...
	$(".sel-this1").click(function(){
		saveRecAddress();
		var sel = $('.sel-this1').find("em").hasClass("checked");
		if(!sel){
			$(".sel-this1 em").addClass("checked");
			$('#fap').data("sel",true);
			$("#invoice").addClass("show_content").removeClass("hide_content");
			console.log($('#fap').data("sel"))
		}else{
			$(".sel-this1 em").removeClass();
			$('#fap').data("sel",false);
	...
	$(".orderSubmit").on('click', function(){
		...
		if($('.text-center').text()==""||$('.text-center').text()=="添加地址"){
			$.toast("请选择地址");
			return;
		...
		var name = $(".recInfo_name").text();  //收货名称
		var phone = $(".recInfo_tel").text();   //电话

		var address = $(".recInfo_addr").text();   //地址

		var zipcode = $(".recInfo_addr").data("code"); //订单编号
		var fapmsg = $('#fap').data("sel");
		var sel = $('.sel-this1').find("em").hasClass("checked");
		var invoce = $("#receiptName").val();// 发票
		var company = $("#picker").val();  // 物流
		var dataID = $("#dataID").val(); //送货时间
		...
		if(name.length==0){
			...
			$.ajax({
		        ...
		        success:function(msg){
	              ...
	              //  window.location.href='/modules/pay/html/order_PayComfirm.html?code='+ m.respData.ordercode +'&pay='+ m.respData.payfee;
